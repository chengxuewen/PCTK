...
#-----------------------------------------------------------------------------------------------------------------------
function(__pctk_internal_static_link_order_public_test result)
    # We could trust iOS linker
    if(IOS)
        set(PCTK_HAVE_LINK_ORDER_MATTERS "FALSE" CACHE INTERNAL "Link order matters")
    endif()

    if(DEFINED PCTK_HAVE_LINK_ORDER_MATTERS)
        set(${result} "${PCTK_HAVE_LINK_ORDER_MATTERS}" PARENT_SCOPE)
        return()
    endif()

    if(EXISTS "${PCTK_CMAKE_DIR}")
        set(test_source_basedir "${PCTK_CMAKE_DIR}/..")
    else()
        set(test_source_basedir "${_pctk_cmake_dir}/${PCTK_CMAKE_EXPORT_NAMESPACE}")
    endif()
    ...
    set(PCTK_HAVE_LINK_ORDER_MATTERS "${${result}}" CACHE INTERNAL "Link order matters")
...
#-----------------------------------------------------------------------------------------------------------------------
# Sets _pctk_link_order_matters flag for the target.
#-----------------------------------------------------------------------------------------------------------------------
function(__pctk_internal_set_link_order_matters target link_order_matters)
    if(NOT TARGET ${target})
        message(FATAL_ERROR "Unable to set _pctk_link_order_matters flag. ${target} is not a target.")
    endif()
    ...
    set_target_properties(${target} PROPERTIES _pctk_link_order_matters "${link_order_matters}")
endfunction()
...
function(__pctk_internal_check_link_order_matters)
    ...
    __pctk_internal_set_link_order_matters(${PCTK_CMAKE_EXPORT_NAMESPACE}::platform "${link_order_matters}")
...
#-----------------------------------------------------------------------------------------------------------------------
function(__pctk_internal_get_cmp0099_genex_check result)
    if(POLICY CMP0099)
...
#-----------------------------------------------------------------------------------------------------------------------
#-----------------------------------------------------------------------------------------------------------------------
function(__pctk_internal_check_cmp0099_available)
    set(platform_target ${PCTK_CMAKE_EXPORT_NAMESPACE}::platform)
    ...
    set_target_properties(${platform_target} PROPERTIES _pctk_cmp0099_policy_check "${cmp0099_check}")
...
#-----------------------------------------------------------------------------------------------------------------------
#-----------------------------------------------------------------------------------------------------------------------
function(__pctk_internal_process_dependency_object_libraries target)
    ...
    get_target_property(processed ${target} _pctk_object_libraries_finalizer_processed)
    if(processed)
        return()
    endif()
    ...
    __pctk_internal_collect_dependency_object_libraries(${target} objects)
...
#-----------------------------------------------------------------------------------------------------------------------
#-----------------------------------------------------------------------------------------------------------------------
function(__pctk_internal_collect_dependency_object_libraries target out_var)
    set_property(GLOBAL PROPERTY _pctk_processed_object_libraries "")

    __pctk_internal_collect_object_libraries_recursively(object_libraries ${target} ${target})
...
#-----------------------------------------------------------------------------------------------------------------------
#-----------------------------------------------------------------------------------------------------------------------
function(__pctk_internal_collect_dependency_plugin_object_libraries target plugin_targets out_var)
    __pctk_internal_get_cmp0099_genex_check(cmp0099_check)
    set(plugin_objects "")
...
#-----------------------------------------------------------------------------------------------------------------------
#-----------------------------------------------------------------------------------------------------------------------
function(__pctk_internal_collect_object_libraries_recursively out_var target initial_target)
    get_property(processed_object_libraries GLOBAL PROPERTY _pctk_processed_object_libraries)
    ...
    foreach(lib IN LISTS libs interface_libs)
        ...
        if(TARGET ${lib})
            ...
            if(${lib} IN_LIST processed_object_libraries)
                ...
                set_property(GLOBAL APPEND PROPERTY _pctk_processed_object_libraries ${lib})
            endif()

            get_target_property(is_pctk_propagated_object_library ${lib} _is_pctk_propagated_object_library)
            if(is_pctk_propagated_object_library)
...
#-----------------------------------------------------------------------------------------------------------------------
#-----------------------------------------------------------------------------------------------------------------------
function(__pctk_internal_promote_target_to_global target)
...
#-----------------------------------------------------------------------------------------------------------------------
#-----------------------------------------------------------------------------------------------------------------------
function(__pctk_internal_promote_target_to_global_checked target)
    # With CMake version 3.21 we use a different mechanism that allows us to promote all targets
    # within a scope.
    if(PCTK_PROMOTE_TO_GLOBAL_TARGETS AND CMAKE_VERSION VERSION_LESS 3.21)
        __pctk_internal_promote_target_to_global(${target})
    endif()
endfunction()
...
#-----------------------------------------------------------------------------------------------------------------------
#-----------------------------------------------------------------------------------------------------------------------
function(__pctk_internal_promote_targets_in_dir_scope_to_global)
...
#-----------------------------------------------------------------------------------------------------------------------
#-----------------------------------------------------------------------------------------------------------------------
function(__pctk_internal_promote_targets_in_dir_scope_to_global_checked)
    if(PCTK_PROMOTE_TO_GLOBAL_TARGETS)
        __pctk_internal_promote_targets_in_dir_scope_to_global()
    endif()
endfunction()
...
function(__pctk_internal_defer_promote_targets_in_dir_scope_to_global)
    get_directory_property(is_deferred _pctk_promote_targets_is_deferred)
    if(NOT is_deferred)
        set_property(DIRECTORY PROPERTY _pctk_promote_targets_is_deferred TRUE)

        if(CMAKE_VERSION VERSION_GREATER_EQUAL 3.19)
            cmake_language(DEFER CALL __pctk_internal_promote_targets_in_dir_scope_to_global_checked)
        endif()
    endif()
endfunction()
