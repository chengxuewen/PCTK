...
function(pctk_internal_try_compile_binary_for_strip binary_out_var)
    ...
    # TODO: Clean this up, together with pctk_set_up_build_internals_paths to only use the
    # the pctkbase sources when building pctkbase. And perhaps also when doing a non-prefix
    # developer-build.

    set(config_test_dir "config.tests/binary_for_strip")
    set(pctkbase_config_test_dir "${PCTK_SOURCE_TREE}/${config_test_dir}")
    set(installed_config_test_dir
        "${_pctk_cmake_dir}/${PCTK_CMAKE_EXPORT_NAMESPACE}/${config_test_dir}")

    # pctkbase sources available, always use them, regardless of prefix or non-prefix builds.
    if(EXISTS "${pctkbase_config_test_dir}")
        set(src_dir "${pctkbase_config_test_dir}")

        # pctkbase sources unavailable, use installed files.
    ...
    # happens in PCTK's CI) by choosing a build dir that does not coincide with the installed
    ...
    set(flags "")
    pctk_get_platform_try_compile_vars(platform_try_compile_vars)
    list(APPEND flags ${platform_try_compile_vars})

    # CI passes the project dir of the PCTK repository as absolute path without drive letter:
    #   \Users\pctk\work\pctk\pctkbase
    # Ensure that arg_PROJECT_PATH is an absolute path with drive letter:
    #   C:/Users/pctk/work/pctk/pctkbase
    ...
    try_compile(PCTK_INTERNAL_BUILT_BINARY_FOR_STRIP
        "${binary_dir}"
        "${src_dir}"
...
# The wrapper is used when targeting Linux or MinGW with a shared PCTK build.
...
function(pctk_internal_generate_binary_strip_wrapper)
    ...
    if(DEFINED CACHE{PCTK_INTERNAL_STRIP_SUPPORTS_KEEP_SECTION}
        OR PCTK_NO_STRIP_WRAPPER
        OR (NOT PCTK_BUILD_SHARED_LIBS)
        )
        return()
    endif()
    ...
    if((UNIX OR MINGW)
        ...
        # To make reconfiguration more robust when PCTK_INTERNAL_STRIP_SUPPORTS_KEEP_SECTION is
        # manually removed, make sure to always restore the original strip first, by
        # re-assigning the original value.
        set(CMAKE_STRIP "${PCTK_INTERNAL_ORIGINAL_CMAKE_STRIP}" CACHE STRING "")

        # Getting path to a binary we can run strip on.
        pctk_internal_try_compile_binary_for_strip(valid_binary_path)
        ...
        message(DEBUG
            ...
            "original strip: ${PCTK_INTERNAL_ORIGINAL_CMAKE_STRIP}\n"
        ...
        set(script_name "pctk-internal-strip")
        ...
        # Need to find the libexec input file depending whether the pctkbase sources are available.
        # This mirrors the logic in pctk_set_up_build_internals_paths.
        # TODO: Clean this up, together with pctk_set_up_build_internals_paths to only use the
        # the pctkbase sources when building pctkbase. And perhaps also when doing a non-prefix
        # developer-build.
        set(pctkbase_wrapper_in_path "${PCTK_SOURCE_TREE}/${wrapper_rel_path}")
        set(installed_wrapper_in_path
            "${_pctk_cmake_dir}/${PCTK_CMAKE_EXPORT_NAMESPACE}/${wrapper_rel_path}")

        # pctkbase sources available, always use them, regardless of prefix or non-prefix builds.
        if(EXISTS "${pctkbase_wrapper_in_path}")
            set(wrapper_in "${pctkbase_wrapper_in_path}")
        ...
        set(wrapper_out "${PCTK_BUILD_DIR}/${INSTALL_LIBEXECDIR}/${script_name}${wrapper_extension}")

        # Used in the template file.
        set(original_strip "${PCTK_INTERNAL_ORIGINAL_CMAKE_STRIP}")

        configure_file("${wrapper_in}" "${wrapper_out}" @ONLY)

        # Override the strip binary to be used by CMake install target.
        set(CMAKE_STRIP "${wrapper_out}" CACHE INTERNAL "Custom PCTK strip wrapper")

        message(STATUS "CMAKE_STRIP (used by PCTK): ${CMAKE_STRIP}")
    endif()
endfunction()
...
function(pctk_enable_separate_debug_info target installDestination)
    ...
    if(NOT PCTK_FEATURE_separate_debug_info)
        return()
    endif()
    ...
    get_property(skip_separate_debug_info DIRECTORY "${target_source_dir}" PROPERTY _pctk_skip_separate_debug_info)
    ...
    if(APPLE)
        ...
        configure_file(
            "${CMAKE_CURRENT_FUNCTION_LIST_DIR}/PCTKSeparateDebugInfo.Info.plist.in"
            "Info.dSYM.plist"
        )
        ...
        if(arg_PCTK_EXECUTABLE AND PCTK_FEATURE_debug_and_release)
            ...
            foreach(cmake_config ${cmake_configs})
                ...
                if(NOT (cmake_config STREQUAL PCTK_MULTI_CONFIG_FIRST_CONFIG))
                    set(install_optional_arg OPTIONAL)
                else()
                    unset(install_optional_arg)
                endif()
                pctk_install(DIRECTORY ${debug_info_bundle_dir}
        ...
        pctk_install(FILES ${debug_info_target} DESTINATION ${installDestination})
    endif()
    list(APPEND commands
