...
# A finalizer that is registered with pctk_add_list_file_finalizer(foo bar baz)
...
# For CMake < 3.18 the function pctk_watch_current_list_dir must know about the finalizer.
#
...
function(pctk_add_list_file_finalizer func)
    ...
    if(use_cmake_defer_call)
        ...
        set_property(GLOBAL APPEND
            PROPERTY PCTK_LIST_FILE_FINALIZER_FILES "${CMAKE_CURRENT_LIST_FILE}")
        set_property(GLOBAL APPEND
            PROPERTY PCTK_LIST_FILE_FINALIZER_FUNCS ${func})
        foreach(i RANGE 1 9)
            set(arg "${ARGV${i}}")
            if(i GREATER_EQUAL ARGC OR "${arg}" STREQUAL "")
                set(arg "IGNORE")
            endif()
            set_property(GLOBAL APPEND PROPERTY PCTK_LIST_FILE_FINALIZER_ARGV${i} "${arg}")
        endforeach()
    endif()
endfunction()
...
function(pctk_watch_current_list_dir variable access value current_list_file stack)
    ...
    get_property(files GLOBAL PROPERTY PCTK_LIST_FILE_FINALIZER_FILES)
    if(NOT files)
        return()
    endif()
    get_property(funcs GLOBAL PROPERTY PCTK_LIST_FILE_FINALIZER_FUNCS)
    foreach(i RANGE 1 9)
        get_property(args${i} GLOBAL PROPERTY PCTK_LIST_FILE_FINALIZER_ARGV${i})
    endforeach()
    list(LENGTH files n)
    set(i 0)
    while(i LESS n)
        ...
        if(file STREQUAL stack_top)
            ...
            if(${CMAKE_VERSION} VERSION_LESS "3.18.0")
                ...
                if(func STREQUAL "pctk_finalize_module")
                    pctk_finalize_module(${a1} ${a2} ${a3} ${a4} ${a5} ${a6} ${a7} ${a8} ${a9})
                elseif(func STREQUAL "pctk_finalize_plugin")
                    pctk_finalize_plugin(${a1} ${a2} ${a3} ${a4} ${a5} ${a6} ${a7} ${a8} ${a9})
                elseif(func STREQUAL "pctk_internal_finalize_app")
                    pctk_internal_finalize_app(${a1} ${a2} ${a3} ${a4} ${a5} ${a6} ${a7} ${a8} ${a9})
                elseif(func STREQUAL "pctk_internal_export_additional_targets_file_finalizer")
                    pctk_internal_export_additional_targets_file_finalizer(
                        ${a1} ${a2} ${a3} ${a4} ${a5} ${a6} ${a7} ${a8} ${a9})
                else()
                    message(FATAL_ERROR "pctk_watch_current_list_dir doesn't know about ${func}. Consider adding it.")
                endif()
            else()
    ...
    endwhile()
    set_property(GLOBAL PROPERTY PCTK_LIST_FILE_FINALIZER_FILES ${files})
    set_property(GLOBAL PROPERTY PCTK_LIST_FILE_FINALIZER_FUNCS ${funcs})
    foreach(i RANGE 1 9)
        set_property(GLOBAL PROPERTY PCTK_LIST_FILE_FINALIZER_ARGV${i} "${args${i}}")
    endforeach()
endfunction()
