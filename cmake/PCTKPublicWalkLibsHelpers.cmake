...
function(__pctk_internal_walk_libs target out_var rcc_objects_out_var dict_name operation)
    ...
    if(target STREQUAL "${PCTK_CMAKE_EXPORT_NAMESPACE}::EntryPointPrivate")
    ...
    __pctk_internal_get_dict_key_values(libs "${target}" "${dict_name}" "libs")
    __pctk_internal_get_dict_key_values(rcc_objects "${target}" "${dict_name}" "rcc_objects")
    ...
    if(libs MATCHES "-NOTFOUND$")
        ...
        # build purposes.
        get_target_property(main_target_rcc_objects ${target} _pctk_rcc_objects)
        if(main_target_rcc_objects)
            __pctk_internal_merge_libs(rcc_objects ${main_target_rcc_objects})
        endif()
        ...
        foreach(lib ${target_libs})
            ...
            # Skip static plugins.
            set(_is_plugin_marker_genex "\\$<BOOL:PCTK_IS_PLUGIN_GENEX>")
            ...
            elseif(TARGET ${lib_target})
                ...
                if(lib_target_type STREQUAL "INTERFACE_LIBRARY")
                    if(NOT ${direct})
                        __pctk_internal_walk_libs(
                            ${lib_target}
                            lib_libs_${target}
                            lib_rcc_objects_${target}
                            "${dict_name}" "${operation}" ${collected})
                        if(lib_libs_${target})
                            __pctk_internal_merge_libs(libs ${lib_libs_${target}})
                            set(is_module 0)
                        endif()
                        if(lib_rcc_objects_${target})
                            __pctk_internal_merge_libs(rcc_objects ${lib_rcc_objects_${target}})
                        endif()
                    else()
                        __pctk_internal_merge_libs(libs ${lib})
                    endif()
                elseif(NOT lib_target_type STREQUAL "OBJECT_LIBRARY")

                    if(operation MATCHES "^(collect|direct)_targets$")
                        __pctk_internal_merge_libs(libs ${lib_target})
                    else()
                        __pctk_internal_merge_libs(libs "$<TARGET_LINKER_FILE:${lib_target}>")
                    endif()

                    get_target_property(target_rcc_objects "${lib_target}" _pctk_rcc_objects)
                    if(target_rcc_objects)
                        __pctk_internal_merge_libs(rcc_objects ${target_rcc_objects})
                    endif()
                ...
                if(operation STREQUAL "promote_global")
                    ...
                    # like with WrapLibClang and Threads in pctktools.
                    pctk_internal_should_not_promote_package_target_to_global(
            ...
            elseif("${lib_target}" MATCHES "^PCTK::(.*)")
                ...
                if(NOT operation MATCHES "^(collect|direct)_targets$")
                    ...
                    __pctk_internal_merge_libs(libs "${final_lib_name_to_merge}")
                endif()
            endif()
        endforeach()
        __pctk_internal_memoize_values_in_dict("${target}" "${dict_name}" "libs" "${libs}")
        __pctk_internal_memoize_values_in_dict("${target}" "${dict_name}" "rcc_objects" "${rcc_objects}")
...
function(__pctk_internal_get_dict_key_values out_var target_infix dict_name dict_key)
...
function(__pctk_internal_memoize_values_in_dict target dict_name dict_key values)
    ...
    if(versionless_alias)
        __pctk_internal_get_dict_key_values(
    ...
    get_target_property(versionfull_alias "${target}" "_pctk_versionfull_alias")
